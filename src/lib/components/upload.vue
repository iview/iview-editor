    ...
    <Upload
        ...
        <slot>
            <Button type="text" size="small" :disabled="uploadStatus === 1" class="i-editor-upload-item" v-if="styles === 1">
                <Tooltip :content="t('upload-img', '上传图片')" transfer>
            ...
            <Button type="text" size="small" class="i-editor-upload-item" v-if="styles === 6">
                <Tooltip :content="t('import-md-document', '导入 Markdown 文件')" transfer>
    ...
    methods: {
        beforeUploadFile (file) {
            if (this.styles === 6) {
                ...
                if (!(/\.md$/.test(file.name)) && file_type !== 'text/markdown') {
                    this.$Notice.warning({
                        title: this.t('file-format-incorrect', '文件格式不正确'),
                        desc: this.t('file-format-incorrect-md-content', `您上传的文件 ${file.name} 格式不符合要求，请上传 .md 格式的文件。`, {
                            fileName: file.name
                        }),
                        duration: 6
                    });
        ...
        handleFormatError (file) {
            this.$Notice.warning({
                title: this.t('file-format-incorrect', '文件格式不正确'),
                desc: this.t('file-format-incorrect-img-content', `您上传的文件 ${file.name} 格式不符合要求，请上传 jpg、png、gif 格式的文件。`, {
                    fileName: file.name
                }),
                duration: 6
            });
        ...
        handleMaxSize (file) {
            this.$Notice.warning({
                title: this.t('file-large', '文件过大'),
                desc: this.t('file-large-content', `您上传的文件 ${file.name} 体积过大，请上传不超过 ${this.config.maxSize / 1024} 的文件。`, {
                    fileName: file.name,
            ...
            if (this.styles === 3) {
                ...
                if (!this.showUploadProgress) {
                    this.showUploadProgress = this.$Message.loading({
                        duration: 0,
                        render: h => {
                            return h('span', this.t('upload-loading', `图片上传中（${this.uploadPercent}%）`, {
